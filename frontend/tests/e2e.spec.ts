...
test.describe('Autenticação e coleções do usuário', () => {
  test('registro, login, persistência, favoritos e equipe', async ({ page }) => {
    ...
    // BUG-01 corrigido: chave correta é 'kogui.auth'
    ...
    // BUG-02 corrigido: o perfil é persistido junto com os tokens após o login
    ...
    // Seletores corrigidos para corresponder ao HTML real (title attribute)
    const favoriteButton = firstCard.locator('button[title="Favoritar"]');
    await favoriteButton.click();

    const teamButton = firstCard.locator('button[title="Adicionar à equipe (máx. 6)"]');
    ...
    expect(parsedAfterReload.user?.username).toBe(username);

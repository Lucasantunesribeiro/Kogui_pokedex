...
import { ApiService, FavoriteItem, Pokemon, TeamSlotItem } from '../../services/api.service';
  ...
  onToggleFavorite(pokemon: Pokemon): void {
    ...
    if (existing) {
      this.collections.removeFavorite(existing).subscribe({
        next: () =>
          this.feedback.notifyInfo(`${this.formatName(pokemon.name)} removido dos favoritos.`),
    ...
    this.collections.addFavorite(pokemon.id).subscribe({
      next: () =>
        this.feedback.notifySuccess(`${this.formatName(pokemon.name)} foi favoritado!`),
      error: (err) => {
